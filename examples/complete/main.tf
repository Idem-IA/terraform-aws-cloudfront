...
locals {
  domain_name = "Idem-modules.modules.tf" # trimsuffix(data.aws_route53_zone.this.name, ".")
  subdomain   = "cdn"
}
...
module "s3_one" {
  source  = "Idem-modules/s3-bucket/aws"
  version = "~> 5.0"
...
locals {
  package_url = "https://raw.githubusercontent.com/Idem-modules/terraform-aws-lambda/master/examples/fixtures/python-zip/existing_package.zip"
...
module "lambda_function" {
  source  = "Idem-modules/lambda/aws"
  version = "~> 8.0"
...
module "records" {
  source  = "Idem-modules/route53/aws//modules/records"
  version = "~> 5.0"
...
module "ec2" {
  source  = "Idem-modules/ec2-instance/aws"
  version = "~> 6.0"
